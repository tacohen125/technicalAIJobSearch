...
Options:
  ...
  --output-dir <path>   Directory for finished resumes/cover letters
                        (default: assets/outputs/ within skill directory)
...
def _is_blank_template(path: Path) -> bool:
    ...
    Detection rules (any match → template):
      1. Square-bracket placeholders with 8+ chars of content, e.g.
         [FILL IN your full name], [Title of Accomplishment], [Describe the ...],
         [e.g. leadership, technical].  Covers all Markdown reference templates.
      2. Shell config template placeholders: "YourFirstName" / "YourLastName"
         (used in config.template.sh).
      3. Auto-generated machine files written by onboard.py (config.sh) — these
         carry an "auto-generated by onboard.py" comment and are always safe to
         regenerate; never edited by the user.
    """
    try:
        text = path.read_text(encoding="utf-8")
        if text.strip() == "":
            return True
        # Rule 1: Markdown-style bracket placeholders
        if re.search(r'\[[^\]]{8,}\]', text):
            return True
        # Rule 2: Shell template sentinel values
        if "YourFirstName" in text or "YourLastName" in text:
            return True
        # Rule 3: Machine-generated by onboard.py — always safe to overwrite
        if "auto-generated by onboard.py" in text:
            return True
        return False
    except Exception:
        return False
...
def main() -> None:
    ...
    if args.output_dir is not None:
        output_dir_raw = args.output_dir.strip()
        print(f"  Output directory: {output_dir_raw or '(default: assets/outputs/ within skill)'}")
    elif args.force or args.dry_run:
        output_dir_raw = ""
        print("  Output directory: (default — assets/outputs/ within skill directory)")
    else:
        print("  Where should finished resumes and cover letters be saved?")
        print("  Recommended: a folder outside the skill install, e.g.:")
        print("    ~/Documents/JobApplications")
        print("    /c/Users/YourName/OneDrive/Documents/JobApplications")
        print(f"  Press Enter to use the default ({skill_dir / 'assets' / 'outputs'})")
        try:
            resp = input("  Output directory: ").strip()
        except EOFError:
            resp = ""
            print("  (non-interactive: using default assets/outputs/)")
        output_dir_raw = resp

    # Normalise to bash-compatible forward-slash paths (handles Windows C:\ input)
    def _to_bash_path(p: str) -> str:
        ...
        # Expand ~ to the user's home directory (bash can handle ~ natively, but
        # storing an explicit path is safer across environments)
        if p.startswith("~/"):
            from pathlib import Path as _Path
            p = str(_Path.home()).replace("\\", "/") + p[1:]
            p = re.sub(r"^([A-Za-z]):/", lambda m: "/" + m.group(1).lower() + "/", p)
        return p.rstrip("/")

    output_dir_bash = _to_bash_path(output_dir_raw)
    ...
    config_content = dedent(f"""\
        ...
        # Output directory for finished resumes and cover letters.
        # Empty = use assets/outputs/ within the skill directory (the default).
        OUTPUT_DIR="{output_dir_bash}"
    """)
